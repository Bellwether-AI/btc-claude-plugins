# Local App: Baseline (Step 1b) and Verification (Step 5a)
...
- **Step 1b, baseline**, on the unmodified branch before any code changes, so we know what was
  already broken.
- **Step 5a, verification**, on the implemented branch after tests pass, diffed against the
  baseline so only *new* problems block the PR.

Both runs use `${CLAUDE_PLUGIN_ROOT}/scripts/localapp_capture.py`; the diff is
`${CLAUDE_PLUGIN_ROOT}/scripts/localapp_diff.py`. Because one script does both captures, the
normalization that makes the diff work is symmetric by construction, and your job is the judgment
the script cannot make: what command runs this app, whether a blocker is safe to fix yourself,
and what a new warning means for this PR.
...
**Contents:** 1 Detection · 2 Running the capture · 3 Step 1b baseline · 4 Step 5a verification
(pre-flight remediation, blocker gate, no-app gate, diff, warning decisions) · 5 Completion ·
6 PR description · 7 Reference (JSON fields, edge cases)
...
- `local_app_skip: true` → the repo has no runnable app. Step 1b: say so and continue. Step 5a:
...
| `app.py` / `wsgi.py` with flask | `python_flask` | `flask run` | 5000 |
...
- Nothing detected and no cached config: Step 1b writes nothing and moves on; Step 5a asks the
...
```bash
CAP="${CLAUDE_PLUGIN_ROOT}/scripts/localapp_capture.py"
$CAP --mode baseline --name api --type azure_functions --command "func start" --issue $ISSUE_NUMBER
$CAP --mode verify   --name api --type azure_functions --command "func start" --issue $ISSUE_NUMBER
```

- Run it from the repo root of the branch under test (main checkout for baseline, worktree for
  verify). It waits internally, so give the Bash call a `timeout` of
  `(boot_timeout + idle_seconds + 60) * 1000` ms: **240000** with the defaults (60 s boot,
...
- The summary prints the first three errors and, on failure, the last output lines. The full log
  is `.co-dwerker.localapp-<name>-<mode>.log`; read it before deciding anything about a failure.
- Record the PID it prints so a later run can recognise a stale process of ours:
  `checkpoint.py set --append local_app_pids=<pid>` (the script kills its own process group on
  exit, so this matters only if it was itself killed).
...
how many errors and warnings were recorded as pre-existing. No gate. Continue to Step 2.

**No app detected** (nothing to run): say so and continue to Step 2. Step 5a will ask the user
what to do about it.
...
the Step 5a comparison has nothing to compare against, and continuing silently would defeat the
...
1. **Fix the environment and retry (Recommended)** — the user frees the port, starts the
   dependency, or adds the config; when they say ready, re-run Step 1b from detection.
2. **Skip the baseline for this session** — run
   `$CAP --mode baseline --name <app> --type <type> --command "<cmd>" --write-skipped` so Step 5a
   knows there is no baseline, then continue to Step 2. Step 5a will still run, treating every
   error and warning as needing a decision rather than as a regression.
3. **Cancel this session** — exit `/co-dwerker:work` cleanly so they can fix things outside the
   workflow.

Mark `3.1b` completed with `--set baseline_localapp_file=.co-dwerker.baseline-localapp.json`
(or `--set baseline_localapp_file=null` when nothing ran).

---

## 4. Step 5a: verification

This is the last fully local checkpoint before a PR exists. It is a phase gate: Step 6 does not
...
same set of apps Step 1b baselined so the diff is well defined.
...
- **Missing environment variables.** When startup fails on a missing variable, or a template
  ...
  `*_TOKEN`, `*_PASSWORD`, `*_CONNECTION_STRING` with a non-default literal); escalate with the
  list. Record every variable you sourced and from where.
- **Missing tool** (`func`, `dotnet`, `node`, `python`, `docker` not on PATH). Do not install
  anything. Escalate with the tool name and the install command from the repo's `CLAUDE.md` if it
  documents one.
...
1. **I'll fix it, then retry (Recommended)** — pause; when the user says ready, re-run Step 5a
   from detection.
2. **Skip with a documented reason** — ask for a one-line reason (or take it from their reply),
   record it with `checkpoint.py set --set local_app_skip_reason="<reason>"`, and continue to
   Step 6. Step 7 puts the reason in the PR test plan.
...
1. **No runnable app here** — library, CLI, or tooling repo. Merge `"local_app_skip": true` into
   `.co-dwerker.json` (Step 1b and 5a skip cleanly from now on) and treat Step 5a as complete.
2. **Use this command and remember it** — the user gives a command (`make dev`,
   ...
   as `local_app_command`, then run the capture with `--type custom`.
3. **Use a command just this once** — run it now with `--type custom`; do not write config.

### 4.4 Run and diff

```bash
$CAP --mode verify --name <app> --type <type> --command "<cmd>" --issue $ISSUE_NUMBER   # per app
"${CLAUDE_PLUGIN_ROOT}/scripts/localapp_diff.py" diff
```

The diff reads `.co-dwerker.baseline-localapp.json`, `.co-dwerker.verify-localapp.json`, and
`.co-dwerker.json`, prints a per-app report, writes `.co-dwerker.localapp-diff.json`, and exits:

| Exit | Meaning | What you do |
|------|---------|-------------|
| **0** | clean: boot ok, no new errors, no undecided warnings | Step 5a is complete. |
| **2** | block: a healthy→failing boot regression, or new errors | Read the log, fix the cause, re-run Step 5a from the top (`checkpoint.py mark 3.5a in_progress`). Pre-existing errors are listed for information only; never "fix" a baseline error to get a green diff unless it is the point of the issue. |
...
`normalized:` text from the report so the user sees exactly what a permanent dismissal would
match. Options:

1. **Treat as a regression** — collect all such warnings across the prompts, fix them, re-run
   Step 5a.
2. **Dismiss for this PR only** — take a one-line reason and record
   `checkpoint.py set --append dismissed_for_pr='{"warning": "<raw first line>", "reason": "<reason>"}'`.
   The warning stays visible in future sessions.
3. **Dismiss permanently for this repo** — run
   `localapp_diff.py dismiss --normalized "<normalized text>"` immediately, so a re-run in this
   session no longer asks.
...
diff again; it should exit 0.

---

## 5. What "complete" means for Step 5a

Exactly one of:

- **Clean pass** — every app booted healthy, the idle watch ran to completion, `localapp_diff.py`
  exits 0 (after any dismissals).
...
Anything else means Step 5a is still in progress and `checkpoint.py gate 3` will say so.

---

## 6. PR description (Step 7)
...
in-repo file paths, so the same event on two boots produces the same key.
...
| Several apps in one repo | Each has its own diff. Step 5a is complete only when every app is. |
